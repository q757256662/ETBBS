  ...
  <div class="board-item" @click="handleRoute">
    ...
    <div class="detail">
      ...
      <div class="recent">
        ...
        <div class="line">
          <span style="margin-right:40px;">{{topicItem.RewardScore}}</span>
          <span>{{topicItem.keyFileIds}}</span>
        </div>
        <div class="post">
          <!-- <p class="topic-info"> -->
          <span class="replyname-class">{{topicItem.ReplyName}}</span>
          <span class="time">{{topicItem.createTime|formatTime}}</span>
          <!-- </p> -->
        </div>
        <div class="time">
...
export default {
  ...
  watch: {
    parentCheckState(val) {
      this.checkState = val;
      this.$emit("onSettingCheck", this.topicItem, val);
    }
  },
  methods: {
    /**跳转链接 */
    handleRoute() {
      //   this.route.push({path:'/TopicSingle',params:{topicId:topicItem.topicId}});
        this.$router.push({ path: "/TopicSingle?topicId=" + this.topicItem.id});
    },
    onMyCollect() {
      this.$emit("flashPage");
    },
    ...
    handleCheck() {
      ...
      this.$emit("onSettingCheck", this.topicItem, this.checkState);
    }
  }
};
</script>

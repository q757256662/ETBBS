  ...
  <div class="nav">
    ...
    <div class="nav_right">
      ...
      <div class="nav_details">
        ...
        <el-badge :value="dot" class="remindItem" v-if="dot!=0">
          <el-button size="small" style="margin-left:10px;display:block" @click="handleRouter" type="primary">新公告</el-button>
        </el-badge>
        <!-- <li>个人信息</li> -->
        <!-- <router-link to="/dashboard">
...
export default {
  data() {
    return {
      ...
      predefineColors: ["#ffffff", "#ededed", "#cccccc"],
      // dot:0
    };
  },
  computed: {
  ...
  methods: {
    /**跳转路由 */
    handleRouter(){
      this.$router.push({path:'/newTopic/index'})
    },
    /**获取提醒条目 */
    handleGetNewPulic() {
      getNewpublic().then(res => {
        if(res.Success){
          // this.dot = res.Data.Total
          this.$store.commit('SET_DOT',res.Data.Total)
        }
      });
    },
    /**选择背景颜色 */

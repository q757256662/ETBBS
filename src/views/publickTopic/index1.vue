  ...
  <div>
    <div style="background:#fff">
      <div class="fun-search">
        <el-input class="search" placeholder="请输入帖子标题或关键字" suffix-icon="el-icon-search" @keyup.enter.native="handleSearch" v-model="queryList.queryStr" auto-complete="off" size="small"></el-input>
        <el-button size="mini" @click="handleSearch" type="primary">搜索</el-button>
      </div>
      <router-link :class="{'border-item':true,'border-striped':index%2==0}" :to="'/TopicSingle?topicId='+item.Id" v-for="(item,index) in newTopicList" :key="item.Id" tag="a" target="_blank">
